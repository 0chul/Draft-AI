...
export const ResearchAndMatching: React.FC<Props> = ({ analysisData, onNext, agentConfigs }) => {
  ...
  useEffect(() => {
    const processStrategy = async () => {
      // Parallel execution of agents with custom prompts
      const [trendResults, matchResults] = await Promise.all([
        fetchTrendInsights(analysisData.modules, trendAgent?.systemPrompt),
        matchCurriculum(analysisData.modules, matchAgent?.systemPrompt)
      ]);
  ...
  }, [analysisData]); // Added analysisData to deps, technically agentConfigs should be there too but this is a mount effect in this flow
